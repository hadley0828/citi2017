...
public class SupplierController implements ControlledScreen {
    ...
    @FXML
    private TableColumn<RawMaterialInventoryMonitorItemVo,String> raw_material;
    @FXML
...
//        stock_monitor.setEditable(true);
//        raw_material.setCellFactory(TextFieldTableCell.forTableColumn());
//        raw_material.setOnEditCommit(event -> event.getTableView().getItems().get(event.getTablePosition().getRow()).setRaw_material_variety(event.getNewValue()));
        setTable();
        setCell();
        setBar();
    }
