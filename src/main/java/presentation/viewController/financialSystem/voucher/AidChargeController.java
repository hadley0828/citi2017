...
public class AidChargeController implements Initializable, ControlledScreen {
    @FXML
...
//        creditItemService.SaveCreditItem(list, company_id, voucher_id);
    ...
    private void OnAddRow() {
        if (data.size() > 1)
            data.add(data.size() - 2, new DealStateModel("应收账款-", "", "", "", "", "", ""));
        else
            data.add(0, new DealStateModel("应收账款-", "", "", "", "", "", ""));
    }
