...
public class ProfitTableImpl implements ProfitTableService{
	...
	public ProfitTableVo BusinessIncome_period(String time) {
		...
		temp1=helper.CreditCal(DATA.getVourchersByPeriod(time, "5401"));
		temp2=helper.CreditCal(DATA.getVourchersByPeriod(time, "5451"));
		...
		Business_Taxes_and_Surcharges[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "2221"));
				
		double[] Selling_expenses=new double[3];
		Selling_expenses[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5601"));
		Selling_expenses[1]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5601010"));//商品维修费
		Selling_expenses[2]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5601015"))+
				helper.CreditCal(DATA.getVourchersByPeriod(time, "5601016"));//广告费+业务宣传费
		
		
		double[] Management_expenses=new double[4];
		Management_expenses[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5602"));
		Management_expenses[1]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5602009"));//开办费
		Management_expenses[2]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5602002"));//业务招待费
		Management_expenses[3]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5602010"));//研究费用
		
		double[] Financial_expenses=new double[3];//财务费用
		Financial_expenses[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5603"));
		temp1=helper.CreditCal(DATA.getVourchersByPeriod(time, "5603001"));//利息费用
		Financial_expenses[1]=temp1+
				helper.CreditCal(DATA.getVourchersByPeriod(time, "5603002"))+
				helper.CreditCal(DATA.getVourchersByPeriod(time, "5603003"))+
				helper.CreditCal(DATA.getVourchersByPeriod(time, "5603004"));
		Financial_expenses[2]=temp1;
		
		double investment_proceeds=helper.CreditCal(DATA.getVourchersByPeriod(time, "5111"));//投资收益
		
		double operating_profit=business_income+helper.CreditCal(DATA.getVourchersByPeriod(time, "5111"))-business_costs-
		...
		Non_operating_income[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5301"));
		Non_operating_income[1]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5301002"));//营业外收入-政府补助
		
		double[] Non_operating_expenses=new double[6];
		Non_operating_expenses[0]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711"));
		Non_operating_expenses[1]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711005"));//坏账损失
		Non_operating_expenses[2]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711007"));//无法收回的长期债券投资损失
		Non_operating_expenses[3]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711008"));//无法收回的长期股权投资
		Non_operating_expenses[4]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711009"));//自然灾害等不可抗因素造成的损失
		Non_operating_expenses[5]=helper.CreditCal(DATA.getVourchersByPeriod(time, "5711010"));//税收滞纳金
		
		double Total_profit=operating_profit+Non_operating_income[0]-Non_operating_expenses[0];
		
		double Income_tax_expense=helper.CreditCal(DATA.getVourchersByPeriod(time, "5801"));
