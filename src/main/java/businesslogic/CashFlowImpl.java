...
public class CashFlowImpl implements CashFlowTableService{
	...
	public CashFlowVo CashFlowTable_month(String time) {
		...
		Map<String,double[]> tool=helper.tempCal(time);
		double temp1=0,temp2=0,temp3=0,temp4=0,temp5=0,temp6=0;
		
		
		double[] operating_activities=new double[7];
		temp1=helper.Cal(DATA.getVourchersByPeriod(time, "5001"));//主营业务收入
		temp2=helper.Cal(DATA.getVourchersByPeriod(time, "5051"));//其他业务收入
		...
		temp1=helper.DebitCal(DATA.getVourchersByPeriod(time, "1403"))+
				+helper.DebitCal(DATA.getVourchersByPeriod(time, "1405"));//原材料、低值易耗品、包装物、库存商品的借方发生额
		...
		temp2=helper.Cal2(DATA.getVourchersByPeriod(time, "222100101"));//“应交税费-应交增值税-进项税额”	
		operating_activities[4]=temp1-temp2;//1.5“支付的税费”
		
		temp1=helper.Cal2(DATA.getVourchersByPeriod(time, "5711"))-2*helper.Cal2(DATA.getVourchersByPeriod(time, "5711001"));//营业外支出（-其中的非流动资产处置净损失）
		temp2=helper.Cal2(DATA.getVourchersByPeriod(time, "5602"))-2*helper.Cal2(DATA.getVourchersByPeriod(time, "5602001"))-
				2*helper.Cal2(DATA.getVourchersByPeriod(time, "5602007"));//管理费用(-其中的应付职工薪酬、折旧费)
		temp3=helper.Cal2(DATA.getVourchersByPeriod(time, "5601"))-2*helper.Cal2(DATA.getVourchersByPeriod(time, "5601001"));//销售费用 (-其中的应付职工薪酬)
		temp4=helper.DebitCal(DATA.getVourchersByPeriod(time, "1221"));//其他应收款本期借方发生额
		temp5=helper.DebitCal(DATA.getVourchersByPeriod(time, "2241"));//其他应付款本期借方发生额
		temp6=helper.Cal2(DATA.getVourchersByPeriod(time, "5603002"));//财务费用——手续费
		...
		temp1=helper.Cal(DATA.getVourchersByPeriod(time, "5111"));//投资收益
		...
		temp1=helper.CreditCal(DATA.getVourchersByPeriod(time, "1606"));//“固定资产清理”的贷方余额
		...
		temp4=helper.sumList(DATA.getGivenVourchers(time, "1511", "5111"));//长期股权投资形成的投资收益
		if(temp2<0)temp2=0;
		else temp2-=temp4;
		temp3=tool.get("长期债券投资")[1]-tool.get("长期债券投资")[0];//〔长期债券投资期末数－长期债权投资期初数）（当期初数＜期末数时）
		temp5=helper.sumList(DATA.getGivenVourchers(time, "1501", "5111"));//长期债券投资形成的投资收益
		if(temp3<0)temp3=0;
		else temp3-=temp5;
		...
		Financing_activities[1]=helper.Cal(DATA.getVourchersByPeriod(time, "3001"));//“吸收投资者投资收到的现金”
		
		temp1=//“利润分配-应付利润”本期借方发生额中以现金支付的部分
		Financing_activities[3]=helper.sumList(DATA.getGivenVourchers(time, "2231", "1001"))
				+helper.sumList(DATA.getGivenVourchers(time,"2231", "1002"));//3.4偿还借款利息支付的现金
		...
		Net_cash_increase[1]=new BalanceSheetImpl().getDollarAssent(last)[0];
	...
	public CashFlowVo CashFlowTable_year(String time) {
		...
		double[] operating_activities=new double[7];
		temp1=helper.Cal(DATA.getVourchersByYear(time, "5001"));//主营业务收入
		temp2=helper.Cal(DATA.getVourchersByYear(time, "5051"));//其他业务收入
		...
		temp1=helper.DebitCal(DATA.getVourchersByYear(time, "1403"))+
				+helper.DebitCal(DATA.getVourchersByYear(time, "1405"));//原材料、低值易耗品、包装物、库存商品的借方发生额
		...
		temp2=helper.Cal2(DATA.getVourchersByYear(time, "222100101"));//“应交税费-应交增值税-进项税额”	
		operating_activities[4]=temp1-temp2;//1.5“支付的税费”
		
		temp1=helper.Cal2(DATA.getVourchersByYear(time, "5711"))-2*helper.Cal2(DATA.getVourchersByYear(time, "5711001"));//营业外支出（-其中的非流动资产处置净损失）
		temp2=helper.Cal2(DATA.getVourchersByYear(time, "5602"))-2*helper.Cal2(DATA.getVourchersByYear(time, "5602001"))-
				2*helper.Cal2(DATA.getVourchersByYear(time, "5602007"));//管理费用(-其中的应付职工薪酬、折旧费)
		temp3=helper.Cal2(DATA.getVourchersByYear(time, "5601"))-2*helper.Cal2(DATA.getVourchersByYear(time, "5601001"));//销售费用 (-其中的应付职工薪酬)
		temp4=helper.DebitCal(DATA.getVourchersByYear(time, "1221"));//其他应收款本期借方发生额
		temp5=helper.DebitCal(DATA.getVourchersByYear(time, "2241"));//其他应付款本期借方发生额
		temp6=helper.Cal2(DATA.getVourchersByYear(time, "5603002"));//财务费用——手续费
		operating_activities[5]=temp1+temp2+temp3+temp4+temp5+temp6;//1.6“支付其他与经营活动有关的现金”		
		...
		temp1=helper.Cal(DATA.getVourchersByYear(time, "5111"));//投资收益
		temp2=tool.get("应收利息")[1]-tool.get("应收利息")[2];//（应收利息期末数－应收利息期初数）
		temp3=tool.get("应收股利")[1]-tool.get("应收股利")[2];//（应收股利期末数－应收股利期初数）
		Investment_activities[1]=temp1-temp2-temp3;//2.2“取得投资收益收到的现金”
		
		temp1=helper.CreditCal(DATA.getVourchersByYear(time, "1606"));//“固定资产清理”的贷方余额
		...
		temp4=helper.sumList(DATA.getGivenVourchersByYear(time, "1511", "5111"));//长期股权投资形成的投资收益
		if(temp2<0)temp2=0;
		else temp2-=temp4;
		temp3=tool.get("长期债券投资")[1]-tool.get("长期债券投资")[2];//〔长期债券投资期末数－长期债权投资期初数）（当期初数＜期末数时）
		temp5=helper.sumList(DATA.getGivenVourchersByYear(time, "1501", "5111"));//长期债券投资形成的投资收益
		if(temp3<0)temp3=0;
		else temp3-=temp5;
		Investment_activities[3]=temp1+temp2+temp3;//2.4“短期投资、长期债券投资和长期股权投资支付的现金”
		...
		Financing_activities[1]=helper.Cal(DATA.getVourchersByYear(time, "3001"));//“吸收投资者投资收到的现金”
		
		temp1=//“利润分配-应付利润”本期借方发生额中以现金支付的部分
		Financing_activities[3]=helper.sumList(DATA.getGivenVourchersByYear(time, "2231", "1001"))
				+helper.sumList(DATA.getGivenVourchersByYear(time,"2231", "1002"));//3.4偿还借款利息支付的现金
